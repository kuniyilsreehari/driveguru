    ...
    return (
        <Popover>
            <PopoverTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full hover:bg-muted/50 group">
                    <Lock className="h-3.5 w-3.5 text-green-500/70 group-hover:text-green-500 transition-colors" />
                </Button>
            </PopoverTrigger>
            <PopoverContent className="w-72 p-0 bg-card border-border rounded-3xl overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200" align="start" sideOffset={10}>
                <div className="p-5 border-b border-border bg-muted/20">
                    <div className="flex items-center gap-3 mb-1">
                        <ShieldCheck className="h-5 w-5 text-green-500" />
                        <h4 className="font-black text-foreground text-sm uppercase italic tracking-widest">Connection is secure</h4>
                    </div>
                ...
                <div className="p-2">
                    <Button variant="ghost" className="w-full justify-start h-12 rounded-xl hover:bg-muted/50 px-3">
                        <Cookie className="h-4 w-4 mr-3 text-muted-foreground" />
                        <div className="text-left">
                            <p className="text-[11px] font-black text-foreground uppercase tracking-tight">Cookies and site data</p>
                    ...
                    <Link href="/terms">
                        <Button variant="ghost" className="w-full justify-start h-12 rounded-xl hover:bg-muted/50 px-3">
                            ...
                            <div className="text-left">
                                <p className="text-[11px] font-black text-foreground uppercase tracking-tight">About DriveGuru</p>
...
function NotificationCenter() {
    ...
    return (
        <Popover open={isOpen} onOpenChange={setIsOpen}>
            ...
            <PopoverContent className="w-80 p-0 bg-card border-border rounded-3xl overflow-hidden shadow-2xl" align="end">
                <div className="p-4 border-b border-border bg-muted/20">
                    <h4 className="font-black text-foreground text-xs uppercase italic tracking-widest">Activity</h4>
                </div>
                        ...
                        <div className="divide-y divide-border">
                            {notifications.map((notif) => (
                                <Link 
                                    key={notif.id} 
                                    href={notif.link} 
                                    className={cn("flex items-start gap-3 p-4 transition-all hover:bg-muted/50", !notif.read && "bg-orange-500/5")}
      ...
      <div className="container flex h-16 items-center justify-between px-4 max-w-7xl mx-auto">
        <div className="flex items-center gap-1 sm:gap-2">
            <Link href="/" className="flex items-center gap-2">
                ...
                <span className="hidden sm:inline-block font-black text-xl tracking-tighter text-foreground uppercase italic">DriveGuru</span>
            </Link>
            <SiteInfoPopover />
        </div>
        ...
        <nav className="hidden lg:flex items-center gap-1">
            {navItems.map((item) => (
                <Button key={item.href} asChild variant="ghost" size="sm" className={cn("rounded-xl h-9 px-4 font-black uppercase text-[10px] tracking-widest", pathname === item.href ? "bg-muted text-foreground" : "text-muted-foreground hover:bg-muted/50 hover:text-foreground")}>
              ...
              {!isUserLoading && user ? (
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" className="relative h-10 w-10 rounded-full ring-2 ring-border/50 hover:ring-orange-500/50 transition-all p-0 overflow-hidden">
                    ...
                    <DropdownMenuContent className="w-56 p-2 bg-card border-border rounded-2xl shadow-2xl" align="end">
                        ...
                        <DropdownMenuItem onClick={() => router.push(dashboardPath)} className="font-bold h-11 rounded-xl focus:bg-muted/50 cursor-pointer">
                ...
                <div className="flex items-center gap-2">
                    <Button asChild variant="ghost" size="sm" className="font-black text-[10px] uppercase tracking-widest rounded-xl hover:bg-muted/50"><Link href="/login">Login</Link></Button>
                    <Button asChild size="sm" className="font-black text-[10px] uppercase tracking-widest bg-orange-500 hover:bg-orange-600 text-white rounded-xl px-5 h-9 shadow-lg shadow-orange-500/20"><Link href="/signup/role">Join</Link></Button>
                </div>
              )}

              <Button variant="ghost" size="icon" className="h-9 w-9 rounded-full hover:bg-muted/50" onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}>
